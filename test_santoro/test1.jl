...
function test_QSA(;M::Integer = 32, tag = :rrr, τ::Integer = 10^2, seed = 78821000027346)
    #fname = "80x80_couplings.dat"
    fname = "1000_uniform_systems/text_files_and_gs/80x80_uniform1.txt"
...
function test_QSA_alt(;M::Integer = 32, tag = :rrr, τ::Integer = 10^2, seed = 78821000027346, num_steps::Integer = 30)
    #fname = "80x80_couplings.dat"
    fname = "1000_uniform_systems/text_files_and_gs/80x80_uniform1.txt"
    #Xs = RRRMC.GraphEAContSimple(fname)
    #N = RRRMC.getN(Xs)
    ...
    function gen_hook(tag, β, Γ, fst, t0, it0)
        #isdir(dirname) || mkdir(dirname)
        #fn = gen_fname(tag, seed)
        fn = "test_QSA_$(tag)_tau$(τ)_seed$(seed).ALT.txt"
        if fst
            !force && isfile(fn) && error("file $fn exists")
            f = open(fn, "w")
            println(f, "#it acc E time QE β Γ")
    ...
    for s = 1:num_steps
        ...
        sMC = (tag == :rrr && userrr) ? RRRMC.rrrMC : RRRMC.standardMC
        hook, cleanup, t0 = gen_hook(string(tag), β, Γ[s], fst, t0, it0)
        try
            X = RRRMC.GraphQEAT(fname, M, Γ[s], β)
