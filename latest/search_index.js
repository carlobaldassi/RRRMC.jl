...

{
    ...
    "title": "Usage",
    "category": "section",
    "text": "The module is loaded as any other Julia module:julia> using RRRMCThe module provides three functions which implement Monte Carlo Markov Chain algorithms on Ising spin models:standardMC: a standard Metropolis-Hastings sampler\nrrrMC: the reduced-rejection-rate (RRR) method\nbklMC: the Bortz-Kalos-Lebowitz (BKL) methodThe interface for these three algorithms is documented in the Sampling algorithms page, and it is essentially identical: they take as arguments a graph, an inverse temperature parameter β, and the number of Monte Carlo iterations to perform. However, rrrMC and bklMC can only be used on some type of models, see the Graph types page.These functions allow accessing the internal state during the iteration at regular intervals, via the hook keyword argument. They also return the final configuration of the system, which is stored in an object of type Config.The code comes with some built-in graphs, but provides an interface to write user-defined models.note: Note\nThe three sampling functions are the only names exported by the module; all other function and types must be qualified with the RRRMC module name."
},
